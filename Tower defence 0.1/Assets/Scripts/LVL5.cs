...
public class LVL5 : MonoBehaviour
{
            ...
            case 0:
                {
                    ...
                    if (GameObject.Find("Tower_AirU_2(Clone)") == null)
                    {
    ...
    public void Wave()
    {
        if (!IsStart)
            for (int i = 0; i < 4; i++)
            {
                Instantiate(Resources.Load<Transform>("Prefabs/Enemy2"), GameObject.FindWithTag("Start").transform.position, Quaternion.identity);
                IsStart = true;
